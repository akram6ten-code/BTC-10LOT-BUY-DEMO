...
# --- India Demo ka sahi URL (502 fix) ---
        ...
        for sym in list(active_legs.keys()):
            ...
            if not is_reentry:
                if decay_pct >= 0.50 and not data['trailing_active']:
                    ...
                    send_telegram(f"Initial Entry: 50% Profit on {sym}. SL locked at 25% profit.")
            else:
                if decay_pct >= 0.20 and not data['trailing_active']:
                    data['trailing_active'] = True
                    data['current_sl'] = data['entry']
                    send_telegram(f"Re-entry: 20% Profit on {sym}. SL shifted to Cost.")
...
def start_algo():
    send_telegram("Algo Bot Initialized on Delta Demo.")
...
def home():
    try:
        tickers = delta_client.get_tickers()
        return f"BOT LIVE - API OK! Total Tickers: {len(tickers)} - IP Whitelist OK (3 IP Saved)"
    except Exception as e:
...
def start_algo_route():
    ...
    thread.start()
    return "Algo Started! Telegram Check Karo - Trade lagega ab"
